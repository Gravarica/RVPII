...
``` {r}
  ...
  mutate(Response_Time = (unix_timestamp(Arrived_Time) - unix_timestamp(Original_Time_Queued)) / 60)
```

* Plotovanje histograma kako bi se uvidela raspodela vremena odziva: 
``` {r}
df_collected <- df_times %>% sdf_collect()

# Plot histogram with 5-minute bins
ggplot(df_collected, aes(x = Response_Time)) +
  geom_histogram(binwidth = 5, fill = "skyblue", color = "black") +
  labs(title = "Distribution of Response Times",
       x = "Response Time (minutes)",
       y = "Frequency") +
  scale_x_continuous(breaks = seq(0, max(df_collected$Response_Time, na.rm = TRUE), by = 5))
```